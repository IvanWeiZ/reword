...
import { ClaudeProvider } from '../../../src/background/providers/claude';
...
describe('ClaudeProvider', () => {
  ...
  describe('analyzeStreaming', () => {
    it('throws when not configured', async () => {
      ...
      ).rejects.toThrow('Claude client not configured');
    });

    it('streams content and returns parsed result', async () => {
      const chunks = [
        { type: 'content_block_delta', delta: { type: 'text_delta', text: VALID_ANALYSIS_JSON } },
      ];
    ...
    it('accumulates multiple stream chunks', async () => {
      ...
      const chunks = [
        { type: 'content_block_delta', delta: { type: 'text_delta', text: half1 } },
        { type: 'content_block_delta', delta: { type: 'text_delta', text: half2 } },
      ];
      ...
      mockStream.mockResolvedValueOnce({
        [Symbol.asyncIterator]: async function* () {
          yield {
            ...
            delta: { type: 'text_delta', text: VALID_ANALYSIS_JSON },
          };
        },
      });
  ...
  describe('analyzeIncoming', () => {
    ...
    it('parses text block from response', async () => {
      mockCreate.mockResolvedValueOnce({
        content: [{ type: 'text', text: VALID_INCOMING_JSON }],
      });
    ...
    it('handles response with no text block', async () => {
      ...
      provider.configure('sk-test-key');
      // No text block means empty string passed to parser, which will throw
      await expect(provider.analyzeIncoming('hello', [])).rejects.toThrow();
    });
  });
